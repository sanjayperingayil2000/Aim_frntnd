...
import { InspectionStage, CurrentStage } from "../../../components/Charts";
import { Spinner } from "../../../components";
import * as moment from "moment";
import { useNavigate } from "react-router-dom";
import dashboardService from "../../../services/dashboardService";
const AssetConditionAnalysis = () => {
  ...
  const getAssetconditionAnalysis = async () => {
    try {
      let datas;
      setLoading(true);
      let data = {
      ...
      datas = await dashboardService.getAllassetConditionAnalysis(data);
      setLoading(false);
  ...
  const timefilterOnchnage = (timefilter) => {
    ...
    if (timefilter === "Yesterday") {
      ...
      filter = {
        startDate: moment(lastMonday).format(
          "YYYY-MM-DD"
        ),
      ...
      filter = {
        ...
        endDate: moment(getLastDayPreviousMonth(new Date())).format("YYYY-MM-DD"),
      };
  ...
  useEffect(() => {
    getAssetconditionAnalysis();
  }, [values]);

  return (
    <>
      <PageHeader title="Dashboard" subtitle="Asset Condition Analysis">
        <DateRange
      ...
      <Row>
        <Col xl={12} lg={12}>
          <Card
            title="Asset Condition - Inspection Stage"
            variant="chart-card"
            className="mb-4"
          >
            {!loading ? <InspectionStage
              inspectionStage={inspectionStage} values={values} /> : <Spinner />}
          </Card>
          <Card title="Asset Condition - Current Stage" variant="chart-card">
            {!loading ? <CurrentStage
              currentStage={currentStage} values={values} /> : <Spinner />}
          </Card>
        </Col>
      </Row>
      <PageFooter>
        <Button variant="click-primary" size="click-sm" onClick={() => { navigate("/asset-register", { state: { values } }) }}>
          Show Data
        </Button>
      </PageFooter>
    </>
...
export default AssetConditionAnalysis;
